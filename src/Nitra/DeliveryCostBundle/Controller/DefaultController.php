...
//        var_dump($nova_poshta_cost);die;
    ...
    private function calculateDeliveryTimeNovaPoshta($data) 
    {
        ...
        $zone_id = $em->createQuery("
            SELECT p.zone_id FROM NitraDeliveryCostBundle:NovaposhtaTarify p 
            JOIN NitraDeliveryCostBundle:NovaposhtaZone z 
            WITH p.tarif = z.rate
            JOIN z.to_city c  
            JOIN z.from_city h  
            WHERE p.type = 'склад-склад'
            AND ((h.name = :to_city AND c.name = :from_city)
            OR (h.name = :from_city AND c.name = :to_city))")
        ->setParameters(array(
...
//        }
        ...
        if($data['delivery_type'] != 'двери-двери') { 
            ...
            $tarif_for_zone = $em->createQuery('
                    ...
                    WHERE (p.to_city = :to_city AND p.from_city = :from_city) 
                    OR (p.to_city = :from_city AND p.from_city = :to_city)')
                ->setParameters(array(
                    'from_city' => $data['from_city'],
                    'to_city' => $data['to_city']))
                ->setMaxResults(1)
